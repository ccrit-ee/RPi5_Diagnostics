...
class Ethernet_Menu(QWidget):
    def __init__(self,stack):
        super().__init__()
        ethernetLayout = QGridLayout()

        ethernet_stack = QStackedWidget()

        ethernet_stack.addWidget(Ethernet_Step1(ethernet_stack,stack))
        ethernet_stack.addWidget(Ethernet_Step2(ethernet_stack))
        # ethernet_stack.addWidget(Ethernet_Step3(ethernet_stack))
        ethernet_stack.addWidget(Ethernet_Final(ethernet_stack,stack))
        ethernetLayout.addWidget(ethernet_stack)

        self.setLayout(ethernetLayout)

class Ethernet_Step1(QWidget):
    def __init__(self,ethernet_stack,stack):
        super().__init__()
        ...
        title = QLabel("Ethernet Menu")
        title.setStyleSheet("font-size: 20px; font-weight: bold; text-decoration: underline")

        direction = QLabel("Step 1: Locate the ethernet port on the<br>"
                           "Pi 5 and plug one end of the cable into<br>"
                           "it. Then plug the other end into a wall<br>"
                           "port or somewere else with connection.")
        direction.setFixedSize(360,260)
        direction.setStyleSheet("font-size: 18px")

        # Image Setup
        image1_holder = QLabel()
        i1pixmap = QPixmap("images/ethernet_arrow.jpg")
...
class Ethernet_Step2(QWidget):
    def __init__(self,ethernet_stack):
        super().__init__()
        ...
        direction = QLabel("Step 2: Run the test on the next page. The  <br>" 
                           "results will be similar to the image on the <br>"
                           "right. All packets should be sent and recieved <br>"
        ...
        i2pixmap = QPixmap("images/ethernet_results.png")
        ...
        next_btn.clicked.connect(lambda: ethernet_stack.setCurrentIndex(2))
        return_btn.clicked.connect(lambda: ethernet_stack.setCurrentIndex(0))
...
#         next_btn.clicked.connect(lambda: ethernet_stack.setCurrentIndex(3))
#         return_btn.clicked.connect(lambda: ethernet_stack.setCurrentIndex(1))
...
# Widget Configurations, Button Connections ------------------------------------------------------
        menu_title = QLabel("Ethernet Menu") 
        ...
        self.etherNote = QLineEdit()
        self.etherNote.setMaxLength(105)
        self.etherNote.setPlaceholderText("Enter notes here (105 characters max)")
        self.etherNote.setStyleSheet("font-size: 14px")
...
# Layout Configuration ----------------------------------------------------
        ...
        V1.addStretch()
        V1.addWidget(header)
        V1.addWidget(self.etherNote)
        V1.addStretch()
        V1.addLayout(H7)

        self.setLayout(V1)

        ethernet_stack.currentChanged.connect(self.loader)
...
# Command Terminal Test Code --------------------------------------------
    def terminal_command(self):
        ...
        if os == "Windows": #For debugging GUI 
            ...
            command = ["-e", "bash", "-c", f"ping -I eth0 -c 4 google.com; exec bash"]
            self.test.start(self.terminal,command)
            bgtest = subprocess.run(["ping", "-I", "eth0", "-c", "4", "google.com"], capture_output=True, text=True)
        ...
        if self.pass1.isChecked():
            if self.failflag == 0:
                ...
                chk = QMessageBox.question(self,"Change Confirmation",
                                           "Are you sure you want to edit Transmit?", 
...
# JSON save/load --------------------------------------------------------------------------------
    ...
    def load_settings(self):
        ...
        ether = data.get("ether_data", {})
        
        if ether.get("transmit") == "Pass":
            self.pass1.setChecked(True)
        elif ether.get("transmit") == "Fail":
            self.fail1.setChecked(True)
        elif ether.get("usb 2.0 (top)") == "None":
            self.pass1.setChecked(False)
            self.fail1.setChecked(False)
            
        if ether.get("received") == "Pass":
            self.pass2.setChecked(True)
        elif ether.get("received") == "Fail":
            self.fail2.setChecked(True)
        elif ether.get("received") == "None":
            self.pass2.setChecked(False)
            self.fail2.setChecked(False)

        if ether.get("packet loss") == "Pass":
            self.pass3.setChecked(True)
        elif ether.get("packet loss") == "Fail":
            self.fail3.setChecked(True)
        elif ether.get("packet loss") == "None":
            self.pass3.setChecked(False)
            self.fail3.setChecked(False)
            
        self.etherNote.setText(ether.get("etherNote"))
